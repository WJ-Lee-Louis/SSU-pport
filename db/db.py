...
class BaseDB:
    ...
    def __del__(self):
        try:
            ...
            # SQLite 스레드 오류는 무시 (프로그램 종료 시 발생 가능)
            pass
    ...
    def _config_data_based(self, path: str = "./db/notice.db"):
        ...
        self.db_path = Path(path)
        conn = sqlite3.connect(self.db_path, autocommit=True, check_same_thread=False)
        cursor = conn.cursor()
...
class WebsiteDB(BaseDB):
    ...
    def get_notification_categories(self):
        ...
        try:
            ...
            for row in dropdown_notifications:
                id, title, url, college, department, major = row
    ...
    def get_user_notifications(self, user_id: str, limit: int = 20, offset: int = 0) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            notifications.sort(key=lambda x: x.get('timestamp') or '0000-00-00 00:00:00', reverse=True)
    ...
    def register_user(self, user_id: str, email: str) -> Dict[str, Any]:
        ...
        try:
            cursor.execute('''
                INSERT OR REPLACE INTO user (id, email, subscribe, email_notifications) 
                VALUES (?, ?, 
                    COALESCE((SELECT subscribe FROM user WHERE id = ?), ''),
                    COALESCE((SELECT email_notifications FROM user WHERE id = ?), 1))
    ...
    def get_user_email_notifications(self, user_id: str) -> bool:
        ...
        try:
            cursor.execute('SELECT email_notifications FROM user WHERE id = ?', (user_id,))
            result = cursor.fetchone()
            return bool(result[0]) if result else True  # 기본값은 True
        except Exception as e:
    ...
    def update_user_email_notifications(self, user_id: str, enabled: bool) -> Dict[str, Any]:
        ...
        try:
            cursor.execute('UPDATE user SET email_notifications = ? WHERE id = ?', (enabled, user_id))
...
class EmailDB(BaseDB):
    ...
    def get_all_subscribers_email_initial(self) -> Dict[str, List[int]]:
        ...
        try:
            cursor.execute('''
                ...
                WHERE us.is_active = 1 and u.email_notifications = 1
            ...
            notification_dict = {}
            for notification_id, email in results:
    ...
    def get_all_subscribers_email_second(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Group by email address
            email_dict = {}
            for email, notification_id in results:
