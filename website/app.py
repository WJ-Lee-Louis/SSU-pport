...
@app.route('/api/email-notifications', methods=['GET', 'POST'])
def api_email_notifications():
    if 'user' not in session:
        from flask import abort
        abort(401)
    ...
    if request.method == 'GET':
        try:
            with DB(path=DB_PATH) as db:
                enabled = db.get_user_email_notifications(user_id)
            
            return jsonify({'enabled': enabled})
        except Exception as e:
            print(f"이메일 알림 상태 조회 오류: {e}")
        ...
        try:
            ...
            with DB(path=DB_PATH) as db:
                result = db.update_user_email_notifications(user_id, enabled)
